...
def export_model_info(model_path: str, name: str, output_file: str) -> None:
    print(f"\n{'='*60}")
        ...
        for module_name, module in model.named_modules():
            ...
            if has_params or has_buffers:
                ...
                for param_name, param in module.named_parameters(recurse=False):
                    f.write(
                        f"   ├── Param : {param_name:<10} | "
                        f"shape={str(list(param.shape)):<20} | dtype={param.dtype}\n"
                    )

                for buffer_name, buffer in module.named_buffers(recurse=False):
                    f.write(
                        f"   ├── Buffer: {buffer_name:<10} | "
                        f"shape={str(list(buffer.shape)):<20} | dtype={buffer.dtype}\n"
                    )
